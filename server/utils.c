...
int endswith(const char *str, char *pattern) {
    ...
    if (n < m) return 0;
    for (int i = 0; i < m; i++)
        if (str[i] != pattern[n - m + i]) return 0;
    return 1;
}
...
int pop_path(char *source) {
    if (strlen(source) == 1 && source[0] == '/') return 0;
...
int push_path(char *source, const char *target, int is_dir) {
    ...
    for (int i = 0; i <= m; i++) {
        if (target[i] == '/' || (i == m && target[i - 1] != '/')) {
            ...
            if (!strcmp(buffer, ".")) {
                ...
                if (!pop_path(tmp)) return 0;
            } else {
                for (int j = 0; j < buffer_ptr; j++) tmp[tmp_ptr++] = buffer[j];
            }

            if (i < m && target[i] == '/') tmp[tmp_ptr++] = '/';
            buffer_ptr = 0;
        } else {
    ...
    if (is_dir && tmp[tmp_ptr - 1] != '/') tmp[tmp_ptr++] = '/';
